...
const AssignRescuers = () => {
  const [rescuer] = useState([
    ...
    { id: 6, name: 'Linda Johnson', location: 'Baliuag, Bulacan', status: 'Available', barangay: 'Barangay Tarangnan', contactNumber: '567-890-1234' },
    { id: 7, name: 'Peter Brown', location: 'Plaridel, Bulacan', status: 'Available', barangay: 'Barangay Banga 1st', contactNumber: '678-901-2345' },
    { id: 8, name: 'Karen Davis', location: 'Pulilan, Bulacan', status: 'Available', barangay: 'Barangay Dampol 2nd', contactNumber: '789-012-3456' },
    { id: 9, name: 'David Lee', location: 'Calumpit, Bulacan', status: 'Available', barangay: 'Barangay Meysulao', contactNumber: '890-123-4567' },
    { id: 10, name: 'Emily Chen', location: 'Hagonoy, Bulacan', status: 'Available', barangay: 'Barangay San Agustin', contactNumber: '901-234-5678' },
  ]);
  ...
  const [requests] = useState([
    ...
    { id: 6, location: 'Pandi, Bulacan', time: '10:30 AM', status: 'Pending', type: 'Fire' },
    { id: 7, location: 'Bulakan, Bulacan', time: '10:45 AM', status: 'In Progress', type: 'Medical Emergency' },
    { id: 8, location: 'Obando, Bulacan', time: '11:00 AM', status: 'Pending', type: 'Traffic Accident' },
    { id: 9, location: 'Marilao, Bulacan', time: '11:15 AM', status: 'Completed', type: 'Flooding' },
    { id: 10, location: 'Meycauayan, Bulacan', time: '11:30 AM', status: 'In Progress', type: 'Earthquake' },
  ]);
  ...
  // Paginate data
  const totalRows = rescuer.length;
  const totalPages = Math.ceil(totalRows / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = startIndex + rowsPerPage;
  const paginatedRescuers = rescuer.slice(startIndex, endIndex);
    ...
    <div className="flex flex-col h-100 w-full bg-gray-50">
      ...
      <div className="flex-1 bg-white p-2 sm:p-3 flex flex-col">
        ...
        <div className="flex-1">
          <div className="overflow-x-auto"> {/* Ensure horizontal scrolling */}
              ...
              <tbody>
                {paginatedRescuers.map((rescue, index) => (
                  ...
                  >
                    ...
                    <td className="px-1 py-0.5 sm:px-4 sm:py-2">{rescue.name}</td>
                    <td className="px-1 py-0.5 sm:px-4 sm:py-2">{rescue.location}</td>
                    <td className="px-1 py-0.5 sm:px-4 sm:py-2">{rescue.barangay}</td>
                    <td className="px-1 py-0.5 sm:px-4 sm:py-2">{rescue.contactNumber}</td>
                    <td className={`px-1 py-0.5 sm:px-4 sm:py-2 ${rescue.status === 'Available' ? 'text-[#557C55]' : 'text-[#FA7070]'}`}>
                      {rescue.status}
                    </td>
                  </tr>
                ))}
