...
const TurnIndicator = ({ routeData, routeOpacity, isOnRoute }) => {
  ...
  if (!isOnRoute) {
    return (
      ...
      >
        <div className="flex items-center bg-secondary/80 text-white shadow-2xl rounded-full px-4 py-6 gap-2 md:scale-75">
          <MdWarning className="h-8 w-8 text-white" /> {/* Warning Icon */}
          <span className="flex-1 text-lg font-bold">You're off route!</span>
        </div>
      </div>
    );
  }
